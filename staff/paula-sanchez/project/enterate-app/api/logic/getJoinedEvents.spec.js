...
describe('getJoinedEvents', () => {
    ...
    it('returns events where the user is attendee', () => {
        return Promise.all([
            data.insertEvent(sampleEventData(userId, 'Plan uno')),
            data.insertEvent(sampleEventData(userId, 'Plan dos'))
        ])
            .then(([id1, id2]) => Promise.all([
                logic.joinEvent(userId, id1),
                logic.joinEvent(userId, id2)
            ]))
            .then(() => logic.getJoinedEvents(userId))
            .then(events => {
    ...
    it('returns empty when the user has joined nothing', () => {
        return logic.getJoinedEvents(userId)
    ...
    it('fails on non-existing user', () => {
        ...
        return logic.getJoinedEvents('012345678901234567890123')
